...
export default function ProjectBody({ project }: Prop): JSX.Element {
  ...
  const content = project.content
    ? Array.isArray(project.content)
      ...
      : (project.content as DictProjectContents)[locale!]
    : [];
  const projectContentSplitByOutput = groupSplit(
    content,
    (pc) => pc.type === 'output' || pc.type === 'wideimage'
  );
          ...
          {projectContents.length === 1 && projectContents[0].type === 'output' ? (
            ...
            <img
              ...
              src={
                ...
                (projectContents[0] as ProjectContentWideImage).value.src
              }
              width="320"
              height="200"
              alt={
                (projectContents[0] as ProjectContentWideImage).value.alt
                  ? (projectContents[0] as ProjectContentWideImage).value.alt
                  : ''
              }
            />
                ...
                <Fragment key={contentId}>
                  {content.type === 'texts' ? (
                    ...
                    <p className="project-content-heading">
                      {fmt((content as ProjectContentHeading).value, locale!)}
                    </p>
                  ) : content.type === 'image' ? (
                    <img
                      ...
                      height="200"
                      alt={
                        (content as ProjectContentImage).value.alt
                          ? (content as ProjectContentImage).value.alt
                          : ''
                      }
                    />
                    ...
                    content.type === 'quote' && (
                      <div className="project-quote">
                        <p className="project-quote--content">
                          {fmt((content as ProjectContentQuote).value.content, locale!)}
                        </p>
